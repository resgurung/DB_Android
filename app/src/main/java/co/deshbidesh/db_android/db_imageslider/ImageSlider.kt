package co.deshbidesh.db_android.db_imageslider
...
import co.deshbidesh.db_android.db_imageslider.adapters.ViewPagerAdapter
...
import co.deshbidesh.db_android.db_imageslider.interfaces.TouchListener
...
class ImageSlider @JvmOverloads constructor(
    ...
    fun setImageList(imageList: List<SlideModel>, scaleType: ScaleTypes? = null) {
        ...
        viewPager!!.adapter = viewPagerAdapter
        if (itemClickListener!=null) viewPagerAdapter?.setItemClickListener(itemClickListener)
        imageCount = imageList.size
        if (imageList.isNotEmpty()){
            setupDots(imageList.size)
            if (autoCycle) {
